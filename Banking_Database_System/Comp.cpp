...
Comp::Comp() {
    // Create the hash table with COMP_HASH_TABLE_SIZE empty vectors
    vector<struct Account> empty;
    bankStorage2d.resize(COMP_HASH_TABLE_SIZE, empty);
}

// Sort function
void Comp::sort(vector<int> &v) {
    mergesort(v);
}

// Implementation of merge sort
void Comp::mergesort(vector<int>& v) {
...
void Comp::mergesortsub(vector<int>& v, vector<int>& a, vector <int>& b) {
...
std::vector<int> Comp::getTopK(int k) {
    ...
    vector<int> balances;
    for (int i = 0; i < COMP_HASH_TABLE_SIZE; i++)
...
int Comp::hash(string id) {
    ...
    return ((int) crc16(id)) & (COMP_HASH_TABLE_SIZE-1);
}

// CRC-16-CCITT
// x^16 + x^12 + x^5 + 1
// See https://en.wikipedia.org/wiki/Cyclic_redundancy_check
uint16_t Comp::crc16(string id){
    uint8_t x;
    uint16_t crc = 0xFFFF;
