...
CubicProbing::CubicProbing() {
    ...
    bankStorage1d.resize(CUBIC_HASH_TABLE_SIZE, empty);
}

// Find a free slot and return its hash
// If no slots are free return HASH_NOT_FOUND
int CubicProbing::findFree(string id) {
    // Get the hash
    int h = hash(id);

    // Cubic probe for the next free slot
    for (int i = 0; i < CUBIC_HASH_TABLE_SIZE; i++) {
        // Caution: i*i*i can overflow an int
        long long int l = i;
        l *= i;
        l *= i;
        l += h;
        l = l % CUBIC_HASH_TABLE_SIZE;
        int probe = (int) l;
        // int probe = (h + i*i*i) % CUBIC_HASH_TABLE_SIZE;
...
int CubicProbing::findID(string id) {
    // Get the hash
    int h = hash(id);

    // Cubic probe for the id
    for (int i = 0; i < CUBIC_HASH_TABLE_SIZE; i++) {
        // Caution: i*i*i can overflow an int
        long long int l = i;
        l *= i;
...
void CubicProbing::sort(vector<int> &v) {
...
void CubicProbing::mergesort(vector<int>& v) {
...
void CubicProbing::mergesortsub(vector<int>& v, vector<int>& a, vector <int>& b) {
...
int CubicProbing::hash(string id) {
    return (int) (crc32(id) % CUBIC_HASH_TABLE_SIZE);
}
...
uint32_t CubicProbing::crc32(string id)
{
