...
LinearProbing::LinearProbing() {
    ...
    bankStorage1d.resize(LINEAR_HASH_TABLE_SIZE, empty);
}

// Find a free slot and return its hash
// If no slots are free return HASH_NOT_FOUND
int LinearProbing::findFree(string id) {
    // Get the hash
    int h = hash(id);

    // Linear probe for the next free slot
    for (int i = 0; i < LINEAR_HASH_TABLE_SIZE; i++) {
        int probe = (h+i) % LINEAR_HASH_TABLE_SIZE;
...
int LinearProbing::findID(string id) {
    // Get the hash
    int h = hash(id);
...
// Sort function
void LinearProbing::sort(vector<int> &v) {

    mergesort(v);
}

// Implementation of merge sort
void LinearProbing::mergesort(vector<int>& v) {
...
void LinearProbing::mergesortsub(vector<int>& v, vector<int>& a, vector <int>& b) {
...
int LinearProbing::hash(string id) {
    return (int) (crc32(id) % LINEAR_HASH_TABLE_SIZE);
}
...
uint32_t LinearProbing::crc32(string id)
{
