...
QuadraticProbing::QuadraticProbing() {
    // Create the hash table with empty accounts
    struct Account empty = {"", -1};
    bankStorage1d.resize(QUAD_HASH_TABLE_SIZE, empty);
}

// Find a free slot and return its hash
// If no slots are free return HASH_NOT_FOUND
int QuadraticProbing::findFree(string id) {
    // Get the hash
    int h = hash(id);

    // Quadratic probe for the next free slot
    for (int i = 0; i < QUAD_HASH_TABLE_SIZE; i++) {
        // Caution: i*i can overflow an int
        long long int l = i;
        l *= i;
        l += h;
        l = l % QUAD_HASH_TABLE_SIZE;
        int probe = (int) l;
        // int probe = (h + i*i) % QUAD_HASH_TABLE_SIZE;
        if (bankStorage1d[probe].id == "") {
            return probe;
        }
    }

    // If we got here there were no free slots
    return HASH_NOT_FOUND;
}

// Find an ID
// If not found return HASH_NOT_FOUND
int QuadraticProbing::findID(string id) {
    // Get the hash
    int h = hash(id);

    // Quadratic probe for the id
    for (long i = 0; i < QUAD_HASH_TABLE_SIZE; i++) {
        // Caution: i*i can overflow an int
        long long int l = i;
        l *= i;
        l += h;
...
// We need to provide our own sort function
void QuadraticProbing::sort(vector<int> &v) {
...
void QuadraticProbing::mergesort(vector<int>& v) {
...
void QuadraticProbing::mergesortsub(vector<int>& v, vector<int>& a, vector <int>& b) {
...
void QuadraticProbing::createAccount(string id, int count) {
    // Get the hash
    int h = findFree(id);
    // If there are no free slots this is a fatal error so throw an exception
    if (h == HASH_NOT_FOUND)
...
bool QuadraticProbing::doesExist(string id) {
    return findID(id) != HASH_NOT_FOUND ? true : false;
}
...
bool QuadraticProbing::deleteAccount(string id) {
    ...
    if (h == HASH_NOT_FOUND)
        return false;
    // We delete the account by setting the id and balance to the defaults
    bankStorage1d[h].id = "";
...
int QuadraticProbing::hash(string id) {
    return (int) (crc32(id) % QUAD_HASH_TABLE_SIZE);
}
...
uint32_t QuadraticProbing::crc32(string id)
{
    uint32_t crc = 0xFFFFFFFF;

    for (int i = 0; i < id.size(); i++)
		crc = crc32_tab[(crc ^ id[i]) & 0xFF] ^ (crc >> 8);
